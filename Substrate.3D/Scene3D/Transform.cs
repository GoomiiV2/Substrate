...
namespace Substrate.Scene3D
{
    public struct Transform
    {
        ...
        public void DrawImguiWidget(bool euler = true, bool showMatrix = false)
        {
            ...
            ImGui.SameLine();
            var pos = Position;
            if (ImguiEx.Vector3(ref pos, "Position"))
                Position = pos;

            ImGui.Text("Scale       ");
            ImGui.SameLine();
            var scale = Scale;
            if (ImguiEx.Vector3(ref scale, "Scale"))
                Scale = scale;
            ...
            if (euler)
            {
                ...
                if (ImguiEx.Vector3(ref rot, "Rotation"))
                    RotationEuler = rot;
            }
            else
            {
                var rot = Rotation;
                if (ImguiEx.Quaternion(ref rot, "Rotation"))
                    Rotation = rot;
            }
            ...
            if (showMatrix)
            {
                ...
                ImguiEx.Matrix4x4(ref World, "WorldMatrix");
            }
        }
    }
}
